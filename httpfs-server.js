...
exports.real = function (basePath) {
  ...
  return {
    destpath: (pathItems, cb) => getRealPath(pathItems, cb, (realPath) => cb(0, realPath)),
    getattr: (pathItems, cb) =>
      ...
      getRealPath(pathItems, cb, (realPath) => {
        ...
        fs.open(realPath, "r", (err, fd) => {
          if (err) {
            ...
            fs.read(fd, buffer, 0, length, offset, (err, numw) => {
              if (err) {
                cb(err.errno || unixCodes.ENOENT)
              } else {
      ...
      getRealPath(pathItems, cb, (realPath) => {
        fs.open(realPath, "a", (err, fd) => {
          if (err) {
            ...
            fs.write(fd, buffer, 0, buffer.length, offset, (err, numw) => {
              if (err) {
                cb(err.errno || unixCodes.ENOENT)
              } else {
...
exports.readOnly = function (other) {
  return {
    getattr: (pathItems, cb) =>
      other.getattr(pathItems, (code, attr) => {
        if (code === 0) {
